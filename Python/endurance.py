...
# main function
def endurance(dpg, shift_point, font_resolution):
    throttle_position = 0
    battery_heat = 0
    ice_heat = 0
    charge_level = 0
    ...
    def draw_dashes():
        ...
        # dahses for gas and charge gauges
        dash_point = height - height*(12/16)
        dash_start = width - (width / 10)
        dash_end = width
        dashes.append(dpg.draw_line((dash_start, dash_point), (dash_end, dash_point), color=(255, 255, 255, 255), thickness=4, parent="Window"))
        return dashes
    # draw the rpm box
    def draw_rpm():
    ...
    # Right Side charge gauge
    def draw_charge_gauge():
        bar_width = width / 10
        bar_height =  height*(12/16)*charge_level
        upper_left = (width - bar_width, height - bar_height)
        lower_right = (width, height)
        color = (255*(1-charge_level), 255*charge_level, 0, 255)
        return dpg.draw_rectangle(upper_left, lower_right, color=color, fill=color, parent="Window")
    # Bottom charge gauge
    def draw_GLV_gauge():
        bar_width = (width*0.7778)*GLV_charge
        upper_left = [width / 9, height - (height/50)]
        lower_right = [bar_width + (width / 9), height]
        color = (255*(1-GLV_charge), 255*GLV_charge, 0, 255)
        return dpg.draw_rectangle(upper_left, lower_right, color=color, fill=color, parent="Window")
    # draw the pedal position circle
    def draw_pedal():
        upper_left_y = height/1.2
        upper_left_x = width/2
        radius = height/8
        color = (255*throttle_position, 255*(1-throttle_position), 0, 255)
        pedals = []
        pedals.append(dpg.draw_circle(tag="pedal",center=[upper_left_x,upper_left_y], parent="Window",fill=color,color=color, radius=radius))
        pedals.append(dpg.add_text(parent="Window", pos=(upper_left_x - radius - 20, upper_left_y - radius*1.33), default_value="Pedal Position", tag="pedal text"))
        dpg.bind_item_font(font=dpg.get_alias_id("vera_normal"), item=dpg.get_alias_id("pedal text"))
        return pedals
    def draw_heats():
        upper_left_y = height/1.2
    ...
    while dpg.is_dearpygui_running():
        ...
        heats = draw_heats()
        #draw charge bar
        charge_bar = draw_charge_gauge()
        #draw GLV bar
        GLV_bar = draw_GLV_gauge()
        # render the frame
        dpg.render_dearpygui_frame() 
        # delete all the stuff drawn every frame
        dpg.delete_item(rpm_bar)
        for pedal in pedals:
            dpg.delete_item(pedal)
        dpg.delete_item(charge_bar)
        dpg.delete_item(GLV_bar)
        for dash in dashes:
